...
public class Session implements Protocol, ConfigDefs {
	...
	public static Session create(Event event) throws PushletException, UnsupportedEncodingException {
		Session session;
		try {
			session = (Session) Config.getClass(SESSION_CLASS, "nl.justobjects.pushlet.core.Session").newInstance();
		} catch (Throwable t) {
		...
		// Init session
		session.id = event.getField("userId");
		session.userName = event.getField("userName");
		session.userRealName = event.getField("userRealName");
		session.controller = Controller.create(session);
		session.subscriber = Subscriber.create(session);
		return session;
	}
